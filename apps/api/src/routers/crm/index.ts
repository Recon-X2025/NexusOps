...
export const crmRouter = router({
  ...
  createContact: permissionProcedure("accounts", "write")
    .input(z.object({ firstName: z.string().min(1), lastName: z.string().min(1), email: z.string().email().optional().nullable(), phone: z.string().optional().nullable(), title: z.string().optional().nullable(), accountId: z.string().uuid().optional().nullable() }))
    .mutation(async ({ ctx, input }) => {
      const { db, org } = ctx;
