  ...
  return (
    ...
    >
      ...
      <div className="flex-1 max-w-md mx-4 hidden md:block relative" ref={searchContainerRef}>
        ...
        {showDropdown && (
          <div className="absolute left-0 right-0 top-full mt-1 z-50 bg-popover border border-border rounded-lg shadow-2xl max-h-80 overflow-auto">
            {searchResults.isLoading ? (
              <div className="px-4 py-3 text-xs text-muted-foreground/60">Searching…</div>
            ) : (
              <div className="py-1">
                {Object.entries(groupedResults).map(([type, items]) => {
                  ...
                  return (
                    <div key={type}>
                      <div className="text-[10px] text-muted-foreground/60 uppercase px-3 py-1 tracking-wider">
                        {type.replace(/_/g, " ")}
                      </div>
                      {items.map((item, idx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {item.description && (
                              <span className="text-muted-foreground/60 truncate">{item.description}</span>
                            )}
                          </button>
                        );
                      })}
