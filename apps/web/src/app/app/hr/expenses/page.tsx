...
/**
 * Employee / submitter expense-claims page (canonical path: /app/hr/expenses).
 *
 * This route is the **submission** half of the expenses workflow — file a
 * new claim, edit drafts, submit for approval, track status. Approval and
 * reimbursement live on `/app/finance/expenses` and are gated on
 * `financial.write` so an HR coordinator cannot self-approve their own
 * claim. The legacy `/app/expenses` URL 308-redirects here.
 */
        ...
        <div className="flex items-center gap-2">
          {canApprove && (
            <Link
              ...
              aria-label="Approver queue"
            >
              <CheckCircle2 className="w-3 h-3" /> Approver queue
            </Link>
          )}
                      ...
                      <td className="px-3 py-2.5">
                        <div className="flex items-center gap-1">
                          {c.status === "draft" && canWrite && (
                            <button onClick={() => submitMut.mutate({ id: c.id })} className="px-2 py-0.5 text-[10px] bg-blue-100 text-blue-700 rounded hover:bg-blue-200">Submit</button>
                          )}
