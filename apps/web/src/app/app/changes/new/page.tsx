      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between bg-card border border-border rounded px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-body-sm font-semibold">New Change Request</h1>
          </div>
          <div className="flex items-center gap-2">
            <Link href="/app/changes" className="flex items-center gap-1 rounded border border-border px-3 py-1.5 text-caption hover:bg-accent transition">
        ...
        <div className="bg-card border border-border rounded p-4 flex flex-col gap-3">
          <h2 className="text-caption font-semibold text-muted-foreground uppercase tracking-wider">Change Type</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            {TYPES.map((t) => (
              ...
              >
                <span className={cn("text-caption font-semibold", form.type === t.value ? "text-primary" : "text-foreground")}>{t.label}</span>
                <span className="text-[11px] text-muted-foreground">{t.desc}</span>
              </button>
            ))}
          </div>
          {form.type === "emergency" && (
            <div className="flex items-start gap-2 rounded border border-red-200 bg-red-50 px-3 py-2 text-caption text-red-700">
        ...
        <div className="bg-card border border-border rounded p-4 grid grid-cols-2 gap-3">
          <h2 className="col-span-2 text-caption font-semibold text-muted-foreground uppercase tracking-wider">Change Details</h2>

          <div className="col-span-2 flex flex-col gap-1">
            <label className="text-[11px] font-medium text-muted-foreground">Title *</label>
            <input value={form.title} onChange={(e) => set("title", e.target.value)}
              placeholder="Brief summary of the change…"
              className={cn("rounded border bg-background px-3 py-1.5 text-body-sm focus:outline-none focus:ring-1 focus:ring-primary", errors.title ? "border-red-400" : "border-input")} />
          ...
          <div className="flex flex-col gap-1">
            ...
            <select value={form.riskLevel} onChange={(e) => set("riskLevel", e.target.value)}
              className="rounded border border-input bg-background px-3 py-1.5 text-body-sm focus:outline-none focus:ring-1 focus:ring-primary capitalize">
        ...
        {[
          ...
          <div key={key} className="bg-card border border-border rounded p-4 flex flex-col gap-2">
            <h2 className="text-caption font-semibold text-muted-foreground uppercase tracking-wider">{label}</h2>
            <textarea rows={4} value={(form as Record<string, string>)[key]}
              onChange={(e) => set(key, e.target.value)}
              placeholder={placeholder}
              className={cn("rounded border bg-background px-3 py-1.5 text-body-sm focus:outline-none focus:ring-1 focus:ring-primary resize-none", errors[key] ? "border-red-400" : "border-input")} />
