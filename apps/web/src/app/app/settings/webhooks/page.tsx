...
function DeliveryStatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={cn("rounded-full px-2 py-0.5 text-caption font-medium", map[status] ?? map.pending)}>
      {status}
    </span>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-lg rounded-2xl bg-background border border-border shadow-xl">
        <div className="p-5 border-b border-border">
          <h2 className="text-body-lg font-bold">New Webhook</h2>
        </div>
        <div className="p-5 space-y-4">
          ...
          <div>
            <label className="block text-caption font-medium text-muted-foreground mb-1">Endpoint URL</label>
            <input
              type="url"
              value={url}
          ...
          <div>
            <label className="block text-caption font-medium text-muted-foreground mb-2">Events to subscribe</label>
              ...
              {WEBHOOK_EVENTS.map((ev) => (
                <label key={ev} className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="rounded"
                  />
                  <span className="text-caption font-mono">{ev}</span>
                </label>
              ))}
            </div>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-md rounded-2xl bg-background border border-border shadow-xl">
        <div className="p-5 border-b border-border">
          <h2 className="text-body-lg font-bold">Webhook Secret</h2>
        </div>
        <div className="p-5 space-y-3">
          ...
          <div className="flex items-center gap-2 rounded-lg border border-input bg-muted p-3">
            <code className="flex-1 break-all text-caption font-mono">{secret}</code>
            <button
              type="button"
              onClick={copy}
    ...
    <div className="rounded-xl border border-border bg-card overflow-hidden">
      <div className="flex items-center gap-3 p-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <p className="font-semibold text-body-sm">{hook.name}</p>
            <span className={cn(
          ...
          <p className="text-caption text-muted-foreground font-mono mt-0.5">{hook.url}</p>
          <div className="mt-1 flex flex-wrap gap-1">
            {hook.events.map((ev) => (
              <span key={ev} className="rounded bg-muted px-1.5 py-0.5 text-caption font-mono">{ev}</span>
            ))}
          </div>
        </div>
      ...
      {expanded && (
        <div className="border-t border-border p-4">
          <p className="text-caption font-semibold text-muted-foreground uppercase tracking-wide mb-3">Recent Deliveries</p>
          {loadingDeliveries ? (
            <div className="flex items-center justify-center h-16">
              <Loader2 className="h-5 w-5 animate-spin text-muted-foreground" />
            </div>
          ) : !deliveries || (deliveries as DeliveryRow[]).length === 0 ? (
            <p className="text-caption text-muted-foreground">No deliveries yet.</p>
          ) : (
            <div className="space-y-1.5">
              {(deliveries as DeliveryRow[]).map((d) => (
                <div key={d.id} className="flex items-center gap-3 text-caption">
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-h3 font-bold">Outgoing Webhooks</h1>
