...
import {
  ...
  Clock, Tag, Save, ArrowLeft, Globe, Lock, Loader2, Share2, Briefcase
} from "lucide-react";
...
export default function KnowledgeArticlePage() {
  ...
  const [editMatterId, setEditMatterId] = useState("");

  const utils = trpc.useUtils();
  const { data: matters } = trpc.legal.listMatters.useQuery(
    { limit: 100 },
        ...
        <div className="px-5 py-4 border-b border-border flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-2 flex-wrap">
              ...
              {article.matterId && can("legal", "read") && (
                <span className="flex items-center gap-0.5 text-[10px] px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded border border-blue-200">
                  <Briefcase className="w-2.5 h-2.5" /> Linked Matter
                </span>
              )}
            </div>
          </div>
            ...
            {editing ? (
              ...
              <>
                ...
                <PermissionGate module="knowledge" action="write">
                  ...
                  <button
                    onClick={() => { setEditTitle(article.title); setEditContent(article.content ?? ""); setEditTags(tags.join(", ")); setEditMatterId(article.matterId ?? ""); setEditing(true); }}
                    className="flex items-center gap-1 px-2 py-1 text-[11px] text-muted-foreground border border-border rounded hover:bg-muted/30"
                  >
            ...
            {can("legal", "read") && (
              <div className="px-5 py-3 bg-muted/20 flex items-center gap-2">
                ...
                <select
                  ...
                  className="flex-1 text-[12px] outline-none bg-transparent text-foreground"
                >
                  <option value="">No linked legal matter</option>
                  {matters?.map((m: any) => (
