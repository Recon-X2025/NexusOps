    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-card text-card-foreground rounded-2xl border border-border shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-5 border-b border-border flex-shrink-0">
          <div>
            <h2 className="font-bold text-body-lg">Form Builder</h2>
            <p className="text-caption text-muted-foreground">Designing form for: <strong>{item.name}</strong></p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto flex gap-0">
          ...
          <div className="w-44 border-r border-border p-4 flex-shrink-0">
            <p className="text-caption font-semibold text-muted-foreground mb-3">ADD FIELDS</p>
            <div className="space-y-1">
              {FIELD_TYPES.map(ft => (
                <button
                  key={ft.type}
                  onClick={() => addField(ft.type)}
                  className="w-full flex items-center gap-2 px-3 py-2 text-body-sm rounded-lg hover:bg-primary/10 hover:text-primary transition-colors text-left"
                >
                  <ft.icon className="w-3.5 h-3.5 flex-shrink-0" />
                  <span className="text-caption">{ft.label}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 p-4 space-y-3 overflow-y-auto">
            {fields.length === 0 && (
              <div className="border-2 border-dashed border-border rounded-xl p-10 text-center text-muted-foreground">
                ...
                <p className="text-body-sm">Click field types on the left to add them to the form</p>
              </div>
            )}
            {fields.map((f, idx) => (
              <div key={f.id} className={`border rounded-xl p-4 transition-colors ${editingField === f.id ? "border-primary bg-primary/5" : "border-border bg-card"}`}>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-caption bg-muted px-1.5 py-0.5 rounded font-medium capitalize">{f.type.replace("_"," ")}</span>
                  <span className="flex-1 font-medium text-body-sm">{f.label}</span>
                  {f.required && <span className="text-caption text-red-500">Required</span>}
                ...
                {editingField === f.id && (
                  <div className="space-y-3 mt-3 pl-7">
                    <div>
                      <label className="text-caption font-medium block mb-1">Label *</label>
                      <input value={f.label} onChange={e => updateField(f.id, { label: e.target.value })} className="w-full border border-border rounded-lg px-3 py-1.5 text-body-sm bg-background" />
                    </div>
                    <div>
                      <label className="text-caption font-medium block mb-1">Help Text</label>
                      <input value={f.helpText ?? ""} onChange={e => updateField(f.id, { helpText: e.target.value })} className="w-full border border-border rounded-lg px-3 py-1.5 text-body-sm bg-background" placeholder="Hint for the user" />
                    </div>
                    {(f.type === "dropdown" || f.type === "radio") && (
                      <div>
                        <label className="text-caption font-medium block mb-1">Options (one per line)</label>
                        <textarea rows={3} value={(f.options ?? []).join("\n")} onChange={e => updateField(f.id, { options: e.target.value.split("\n").filter(Boolean) })} className="w-full border border-border rounded-lg px-3 py-1.5 text-body-sm bg-background resize-none" placeholder="Option 1&#10;Option 2&#10;Option 3" />
                      </div>
                    )}
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" checked={f.required} onChange={e => updateField(f.id, { required: e.target.checked })} className="rounded" />
                      <span className="text-caption">Required field</span>
                    </label>
                  </div>
                )}
              </div>
        ...
        <div className="p-5 border-t border-border flex-shrink-0 flex items-center justify-between">
          <p className="text-caption text-muted-foreground">{fields.length} field{fields.length !== 1 ? "s" : ""} configured</p>
          <div className="flex gap-3">
            <button onClick={onClose} className="px-4 py-2 rounded-lg border border-border text-body-sm">Cancel</button>
            <button disabled={saving} onClick={() => onSave(fields)} className="px-4 py-2 rounded-lg bg-primary text-primary-foreground text-body-sm font-medium disabled:opacity-50">
...
export default function CatalogPageClient() {
  ...
  const tabBtn = (key: CatalogView, label: ReactNode) => (
    <button
      ...
      className={cn(
        "rounded-md px-3 py-1.5 text-caption font-medium transition-all",
        view === key
    ...
    <div className="flex min-w-0 max-w-[1600px] flex-col gap-4">
      ...
      <div className="flex flex-col gap-3 border-b border-border pb-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="min-w-0">
            <h1 className="text-body font-semibold tracking-tight text-foreground">Service Catalog</h1>
        ...
        >
          ...
          {can("catalog", "admin") && (
            <button
              ...
              className={cn(
                "flex items-center gap-1 rounded-md px-3 py-1.5 text-caption font-medium transition-all",
                view === "admin"
                  ? "bg-background text-foreground shadow-sm ring-1 ring-border"
                  : "text-muted-foreground hover:text-foreground",
                tabPending && "opacity-60",
            ...
            {CATALOG_CATEGORIES.map((cat) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0">
                    <div className="text-[12px] font-medium text-foreground/80">{cat.label}</div>
          ...
          <div className="overflow-hidden rounded-lg border border-border bg-card">
            ...
            {requestsLoading ? (
              ...
              <div className="px-6 py-14 text-center">
                <Package className="mx-auto mb-2 h-8 w-8 text-muted-foreground/40" />
                <p className="text-body-sm font-medium text-foreground">You have no catalog requests yet</p>
                <p className="mt-1 text-[12px] text-muted-foreground">
      ...
      {view === "admin" && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={s.label} className="bg-card border border-border rounded-xl p-4 flex items-center gap-3">
                ...
                <div><p className="text-h4 font-bold">{s.value}</p><p className="text-caption text-muted-foreground">{s.label}</p></div>
              </div>
            ))}
          </div>
          ...
          {adminQueueEnabled && (
            <div className="bg-card border border-border rounded-xl overflow-hidden">
              <div className="p-4 border-b border-border flex items-center justify-between">
                <h3 className="font-semibold text-body-sm">Service request queue</h3>
                  ...
                  <thead className="bg-muted/50">
                    <tr>
                      {["Item", "Status", "Submitted", "Ticket", "Actions"].map((h) => (
                        <th key={h} className="text-left px-4 py-2 text-caption font-medium text-muted-foreground">
                          {h}
                        </th>
                      ))}
                    </tr>
                      ...
                      ((queueRequests ?? []) as any[]).map((r: any) => {
                        ...
                        return (
                          <tr key={r.id} className="border-t border-border hover:bg-muted/20">
                            ...
                            <td className="px-4 py-2">
                              <span className={`text-caption px-1.5 py-0.5 rounded font-medium ${st?.color ?? "bg-muted text-muted-foreground"}`}>
                                {st?.label ?? r.status}
                              </span>
                            </td>
                            <td className="px-4 py-2 text-caption text-muted-foreground">{formatCatalogDate(r.createdAt)}</td>
                            <td className="px-4 py-2">
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            <div className="p-4 border-b border-border flex items-center justify-between">
              <h3 className="font-semibold text-body-sm">Catalog Items — Form Builder</h3>
              <p className="text-caption text-muted-foreground">Click "Design Form" to add custom fields to any catalog item</p>
            </div>
            <table className="w-full text-body-sm">
              <thead className="bg-muted/50">
                <tr>{["Name","Category","Fields","SLA","Approval","Status","Actions"].map(h => <th key={h} className="text-left px-4 py-2 text-caption font-medium text-muted-foreground">{h}</th>)}</tr>
              </thead>
              <tbody>
                ...
                {catalogItems.map((item: any) => (
                  <tr key={item.id} className="border-t border-border hover:bg-muted/30">
                    ...
                    <td className="px-4 py-3 text-caption capitalize text-muted-foreground">{item.category ?? "—"}</td>
                    <td className="px-4 py-3"><span className="text-caption bg-primary/10 text-primary px-1.5 py-0.5 rounded-full">{(item.formFields ?? []).length} fields</span></td>
                    <td className="px-4 py-3 text-caption">{item.slaDays ?? 3}d</td>
                    <td className="px-4 py-3 text-caption">{item.approvalRequired ? "✓ Required" : "Auto"}</td>
                    <td className="px-4 py-3"><span className={`text-caption px-1.5 py-0.5 rounded capitalize ${item.status === "active" ? "bg-green-100 text-green-700" : "bg-muted text-muted-foreground"}`}>{item.status}</span></td>
                    <td className="px-4 py-3">
                      <button onClick={() => setFormBuilderItem(item)} className="flex items-center gap-1 text-caption text-primary hover:underline font-medium">
