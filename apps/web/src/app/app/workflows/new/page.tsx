      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        <div className="flex items-center justify-between bg-card border border-border rounded px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-body-sm font-semibold">Create Workflow</h1>
          </div>
          <div className="flex items-center gap-2">
            <Link href="/app/workflows" className="flex items-center gap-1 rounded border border-border px-3 py-1.5 text-caption hover:bg-accent transition">
        ...
        <div className="bg-card border border-border rounded p-4 grid grid-cols-2 gap-3">
          <h2 className="col-span-2 text-caption font-semibold text-muted-foreground uppercase tracking-wider">Basic Info</h2>
          <div className="col-span-2 flex flex-col gap-1">
            ...
            <input value={form.name} onChange={(e) => { setForm((f) => ({ ...f, name: e.target.value })); if (errors.name) setErrors((e) => ({ ...e, name: "" })); }}
              ...
              className={cn("rounded border bg-background px-3 py-1.5 text-body-sm focus:outline-none focus:ring-1 focus:ring-primary", errors.name ? "border-red-400" : "border-input")} />
          ...
          <div className="col-span-2 flex flex-col gap-1">
            ...
            <textarea rows={2} value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))}
              ...
              className="rounded border border-input bg-background px-3 py-1.5 text-body-sm focus:outline-none focus:ring-1 focus:ring-primary resize-none" />
          </div>
          <div className="col-span-2 flex items-center gap-2">
            <input type="checkbox" id="isActive" checked={form.isActive} onChange={(e) => setForm((f) => ({ ...f, isActive: e.target.checked }))}
              className="rounded border-input" />
            <label htmlFor="isActive" className="text-caption text-foreground/80 cursor-pointer">Activate immediately after saving</label>
          </div>
        </div>

        {/* Trigger */}
        <div className="bg-card border border-border rounded p-4 flex flex-col gap-3">
          <h2 className="text-caption font-semibold text-muted-foreground uppercase tracking-wider">Trigger</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            {TRIGGERS.map(({ value, label, icon: Icon }) => (
              <button key={value} type="button"
                onClick={() => setForm((f) => ({ ...f, trigger: value }))}
                className={cn("flex items-center gap-2 rounded border p-2.5 text-caption font-medium transition",
        ...
        <div className="bg-card border border-border rounded p-4 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <h2 className="text-caption font-semibold text-muted-foreground uppercase tracking-wider">Action Steps</h2>
            <button type="button" onClick={addStep}
              className="flex items-center gap-1 rounded border border-border px-2.5 py-1 text-caption hover:bg-accent transition">
            ...
            {steps.map((step, i) => (
              <div key={step.id} className="flex items-center gap-2 bg-muted/30 rounded border border-border p-2.5">
                ...
                <input value={step.config} onChange={(e) => updateStep(step.id, "config", e.target.value)}
                  ...
                  className="flex-1 rounded border border-input bg-background px-2 py-1 text-caption focus:outline-none focus:ring-1 focus:ring-primary" />
