...
import {
  ...
  Activity, XCircle, ArrowRight, Printer, FileDown,
...
/**
 * Download the statutory tax invoice as a PDF.
 *
 * `window.print()` (kept alongside) prints the SCREEN — app chrome, sidebar and
 * all — because there is no print stylesheet. This fetches the real document
 * generated by PDFKit on the server.
 *
 * `fetch` rather than `window.open` because the API answers 409 with a JSON
 * explanation when the invoice is not issuable — a payable (vendor) invoice, a
 * missing supplier GSTIN or place of supply, or no line items, which Rule 46
 * requires. `window.open` would dump that JSON into a blank tab.
 */
async function downloadInvoicePdf(id: string, invoiceNumber: string): Promise<void> {
  try {
    const res = await fetch(`/api/financial/invoice-pdf/${id}`, { credentials: "include" });
    if (!res.ok) {
      let message = `Could not generate the tax invoice (HTTP ${res.status}).`;
    ...
    toast.error("Could not reach the server to generate the tax invoice.");
  }
}
