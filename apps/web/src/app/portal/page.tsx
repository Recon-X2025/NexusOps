  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="rounded-xl border border-blue-100 bg-gradient-to-r from-blue-600 to-indigo-600 p-6 text-white">
        <h1 className="text-h4 font-bold">Welcome back, {firstName} 👋</h1>
        <p className="mt-1 text-body-sm text-blue-100">
          How can we help you today? Use the options below to get started.
        </p>
        <div className="mt-3 flex items-center gap-4 text-caption text-blue-200">
          <span>
      ...
      {/* Quick actions */}
      <div>
        <h2 className="mb-3 text-body-sm font-semibold text-gray-700">Quick Actions</h2>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {QUICK_ACTIONS.map(({ href, icon: Icon, label, description, color, iconColor }) => (
            ...
            >
              ...
              <div>
                <p className="text-caption font-semibold leading-tight">{label}</p>
      ...
      <div>
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-body-sm font-semibold text-gray-700">Recent Requests</h2>
          <Link href="/portal/requests" className="text-caption text-blue-600 hover:underline">
            View all →
          </Link>
        </div>
        ...
        {!isLoading && !isError && recentTickets.length === 0 && (
          <div className="rounded-xl border border-dashed border-gray-200 bg-white py-10 text-center">
            ...
            <p className="mt-2 text-body-sm text-gray-500">No requests yet</p>
            <p className="text-caption text-gray-400">
              Submit your first request to get started.
            </p>
            <Link
              href="/portal/request/new"
              className="mt-3 inline-flex items-center gap-1 rounded-md bg-primary px-3 py-1.5 text-caption font-medium text-white hover:bg-primary/90"
            >
                ...
                >
                  <div className="flex flex-col gap-0.5 overflow-hidden">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-body-sm font-medium text-gray-800">{ticket.title}</span>
                    </div>
                  ...
                  <div className="ml-3 flex shrink-0 items-center gap-1.5">
                    ...
                    <span className="text-caption text-gray-600">{status?.name ?? "Open"}</span>
                  </div>
                </div>
              );
            })}
