...
EOF
  ...
  info "Updating AUTH_URL in existing .env.production..."
  # Use | instead of / as delimiter in sed to avoid issues with potential slashes in values
  sed -i "s|^AUTH_URL=.*|AUTH_URL=http://${SERVER_IP}|" "$ENV_FILE"
  sed -i "s|^NEXT_PUBLIC_APP_URL=.*|NEXT_PUBLIC_APP_URL=http://${SERVER_IP}|" "$ENV_FILE"
  
  # If they don't exist yet, append them
  grep -q "AUTH_URL=" "$ENV_FILE" || echo "AUTH_URL=http://${SERVER_IP}" >> "$ENV_FILE"
  grep -q "NEXT_PUBLIC_APP_URL=" "$ENV_FILE" || echo "NEXT_PUBLIC_APP_URL=http://${SERVER_IP}" >> "$ENV_FILE"
  success "AUTH_URL updated in .env.production"
fi
