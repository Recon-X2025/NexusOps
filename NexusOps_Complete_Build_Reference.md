...
**Version:** 4.2  
**Doc revision:** April 4, 2026 — v4.2 reflects: **Phase 3 complete — verified on production (Vultr `139.84.154.78`).** **New modules:** Recruitment/ATS (`recruitment` router, `/app/recruitment`), Corporate Secretarial (`secretarial` router, `/app/secretarial` live API), Workforce analytics (`workforce` router, `/app/people-analytics`). **Schema:** `packages/db/drizzle/0004_recruitment_secretarial.sql` adds **11** tables (5 recruitment + 6 secretarial) and enums; `scripts/apply-phase3-schema.sh` for production DDL. **Fixes:** secretarial meeting create `chairpersonId` → DB `chairperson`; recruitment duplicate application → `CONFLICT`; hire caps `filled`; requisition publish workflow. **Scale:** DB tables **121 → 132**; tRPC routers **39**; procedures **~299** (was 253).  
**Prior (v4.1):** Exhaustive QA run (Suites 05–07) — 261/261 tests pass. 6 production infrastructure bugs found and fixed: 3 missing DB tables (`csm_cases`, `assignment_rules`+`user_assignment_stats`, `salary_structures`+`payroll_runs`+`payslips`) created directly in PostgreSQL. 2 Date serialization code bugs fixed in `walkup.analytics` and `dashboard.getTimeSeries` (raw `Date` object → `.toISOString()::timestamptz`). Admin password hash corrected (`demo1234!`). Total tables: 115 → **121**. All 53 page routes verified. All 253 tRPC procedures verified. All 47 UI interaction tests verified. Platform readiness: **95/100**.  
**Prior (v4.0):** P0/P1 platform fixes deployed. TG-13: Drizzle operator exports. TG-14: surveys RBAC. TG-15: BCRYPT_CONCURRENCY. INFRA-1: covering indexes. P0/P1 platform fixes — all open issues from stress/chaos audit closed. **TG-13**: Drizzle duplicate operator export eliminated from `packages/db/src/index.ts`; `Symbol(drizzle:Columns)` 5xx on ticket/work-order creates for non-admin roles resolved. **TG-14**: `surveys` added as explicit `Module` type; surveys router permission corrected `analytics` → `surveys`; `hr_manager`, `itil`, `itil_admin`, `requester` updated with `surveys` permissions. **TG-15**: `BCRYPT_CONCURRENCY` raised 8 → 32; `LIBUV_THREADPOOL_SIZE=32` in docker-compose — login throughput under load unblocked. **INFRA-1**: 4 covering indexes applied to `tickets` table (`org_sla_breached`, `org_created`, `org_resolved`, `org_status_covering`) — `executiveOverview` timeout resolved. **Infrastructure**: nginx reverse proxy active port 80; web container rebound to `127.0.0.1:3000`; certbot installed (HTTPS-ready); automated pg_dump cron daily 02:00 UTC; disk 85% → 24% (48 GB freed). Prior (v3.9): 12 frontend pages wired to live data (tickets, approvals, security, walk-up, financial, compliance, reports, events, CRM, surveys). Assignee join, priority join, live CSAT/eNPS, executiveOverview live metrics.
