...
**Version:** 3.8  
**Doc revision:** April 3, 2026 — v3.8 reflects: full production clean slate (all transactional data wiped, org_counters reset, ticket/change/WO sequences start from 0). Hardcoded demo data removed from 8 pages (CRM, Contracts, HR, APM, Surveys, Employee Portal, Virtual Agent, Flows). All 50 frontend pages audited and wired — no dead buttons remain. 94-issue frontend audit resolved. Critical backend hardening: `toggleWatch`, `workOrders.update`, `walkup.hold`, `crm.updateLead`, `contracts.completeObligation` mutations added. Session invalidation after password change. Internal endpoint auth. In-flight counter fix. Prior (v3.7): 10,000-session stress test (March 27) and destructive chaos test Round 2 (April 2): 0 HTTP 500s, 0 crashes, 62,369 requests. See `NexusOps_Destructive_Chaos_Test_Report_2026.md` and `NexusOps_Build_Report_20260403.md`.
