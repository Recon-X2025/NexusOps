...
|---------|------|---------|
| **2.2** | 2026-04-28 | **Transitioned to fully dynamic, Faker.js-powered data seeding.** Replaced all hardcoded transactional arrays in `seed.ts`, `seed-modules.ts`, and `seed-smb-analytics.ts` with programmatic generation. Fixed 14+ database enum mismatches (e.g. `cloud_readiness`, `deployment_env`, `lead_status`) to ensure seed integrity. Increased SMB demo dataset to 650 employees for realistic dashboard metrics. |
| **2.1** | 2026-04-26 | **GA-readiness pass for Indian mid-market (full execution log in `docs/MARKET_ASSESSMENT_2026-04-26.md`).** **Surface narrowing:** Walk-Up Experience module retired end-to-end — `/app/walk-up`, `walkup` router, `walkup_*` schema, RBAC entries, sidebar/command-palette wiring, admin toggle, and every e2e/full-QA/chaos/stress fixture removed. Migration `packages/db/drizzle/0028_optimal_sinister_six.sql` drops `walkup_visits` + `walkup_appointments` + the two enums. Walk-in visits now flow as regular `tickets` with `channel = "walk_in"`. **Expense routes split:** employee self-serve (`hr.expenses.createMine` / `listMine`, `protectedProcedure`, employeeId resolved from session) at `/app/hr/expenses`; finance review queue at `/app/finance/expenses` (`financial.write`) with mandatory rejection-reason capture (≥4 chars, shown to employee) via the now-extensible `ConfirmDialog` (`children` + `disableConfirm`). **Integrations admin:** `/app/settings/integrations` dynamically renders forms from a server-side `PROVIDER_CATALOG` (`apps/api/src/routers/integrations.ts`); 12 providers catalogued including **eMudhra Aadhaar e-Sign**, **WhatsApp (AiSensy)**, **SMS (MSG91)**, **Razorpay**, **ClearTax GST IRN**, **Google Workspace**, **M365**. AES-256-CBC encryption at rest with per-tenant DEK wrapped against `INTEGRATIONS_KMS_KEY_ID`. **eMudhra** adapter live (`apps/api/src/services/esign/emudhra.ts`); production credentialing runbook at `docs/EMUDHRA_PRODUCTION_RUNBOOK.md`. **DMS hardening:** `apps/api/src/workflows/virusScanWorkflow.ts` (BullMQ ↔ ClamAV INSTREAM TCP) and `documentRetentionWorkflow.ts` (legal-hold-aware daily sweeper) wired into `services/workflow.ts`; `documents.runRetentionSweepNow` for admin trigger. **Webhook hardening:** `apps/api/src/http/webhooks.ts` enforces (in order) browser-Origin block, OPTIONS rejection, strict CSP/CORS headers, per-provider IP allowlist (`WEBHOOK_ALLOWLIST_*` CIDR-aware env), constant-time HMAC verification. **E2E:** `e2e/esign-webhook.spec.ts` proves the contract → simulated callback → `signature_requests.status = completed` path plus negative paths (bad HMAC → 401, unknown envelope → 404, browser Origin → 403). **Bug fix:** integrations catalog provider key for eMudhra aligned to `emudhra` (was `esign_emudhra`) so the catalog row, `signature_requests.provider`, the `esign.send` integration lookup, and the webhook receiver all converge. Total tables now **119** (was 121 — `walkup_visits` + `walkup_appointments` dropped). |
...
| ~~`walkup`~~ | — | *Retired 2026-04-26 — walk-in visits now use `tickets.channel = "walk_in"`. Migration `0028_optimal_sinister_six.sql` drops the schema.* |
...
| Module | Path | Description |
|--------|------|-------------|
| Overview | `/app/strategy` | Strategy Center hub — executive portfolio surface |
| PMO Workbench | `/app/workbench/pmo` | Daily PMO surface — portfolio matrix, milestones, dependencies |
| Initiatives | `/app/projects` | Initiative portfolio: status, phase, health, milestones, benefits |
| Reports | `/app/reports` | Cross-module reporting & analytics |

> The legacy `/app/strategy-projects` path 308-redirects to `/app/strategy`.
...
| Module | Path | Description |
|--------|------|-------------|
| App Inventory | `/app/apm` | Register of business applications — name, owner, vendor, annual cost, renewal calendar |
| Integrations | `/app/settings/integrations` | Connectors and external system hooks |
| Webhooks | `/app/settings/webhooks` | Outbound event subscriptions |
| API Keys | `/app/settings/api-keys` | Programmatic access tokens |
