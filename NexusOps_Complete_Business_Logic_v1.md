...
|---------|------|---------|
| **1.8** | 2026-04-04 | **Phase 3 business domains live on production (Vultr).** **ATS / Recruitment:** full requisition lifecycle (draft, publish, hold, resume, close); candidate CRM; applications per job with duplicate protection; interview scheduling; offers; hire updates `filled` count (capped). **Secretarial:** board meetings and resolutions, agendas, minutes, ROC-style filings, cap table (share classes), ESOP grant ledger, director register — aligned with India corporate compliance workflows. **Workforce analytics:** org-level headcount, turnover, leave balance aggregates, performance snapshot metrics for HR dashboards. |

| **1.7** | 2026-04-04 | **Exhaustive QA run — all remaining business logic gaps closed.** CSM module: csm_cases table created — all 6 CSM procedures now functional. HR Payroll: salary_structures, payroll_runs, payslips tables created — hr.payroll.listPayslips and salary structure management now functional. Work Orders + Assignment: assignment_rules and user_assignment_stats tables created — workOrders.create/update and all assignment routing logic now operational. Analytics bugs fixed: walkup.analytics and dashboard.getTimeSeries Date serialization corrected. All 253 API procedures verified — zero 500s remain. RBAC: all procedures return correct responses for admin role. Platform: **95/100 readiness**. |
