...
export const legalMatters = pgTable(
  "legal_matters",
  {
    ...
    limitationDeadlineAt: timestamp("limitation_deadline_at", {
      withTimezone: true,
    }),
  ...
  (t) => ({
    orgNumberIdx: uniqueIndex("legal_matters_org_number_idx").on(
      t.orgId,
      t.matterNumber,
    ),
    orgIdx: index("legal_matters_org_idx").on(t.orgId),
    statusIdx: index("legal_matters_status_idx").on(t.orgId, t.status),
    nextHearingIdx: index("legal_matters_next_hearing_idx").on(
      t.orgId,
      t.nextHearingAt,
    ),
  }),
...
export const legalMattersRelations = relations(legalMatters, ({ one }) => ({
  org: one(organizations, {
    fields: [legalMatters.orgId],
    references: [organizations.id],
  }),
  assignedTo: one(users, {
    fields: [legalMatters.assignedTo],
    references: [users.id],
  }),
}));
