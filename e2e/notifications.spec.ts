/** Notifications (Seq 6 C4) — @see docs/QA_NOTIFICATIONS_ITSM_E2E_TEST_PACK.md */
...
async function loginAs(page: Page, email: string, password = "demo1234!") {
  ...
  await page.waitForURL(/app\/command/, { waitUntil: "commit", timeout: 15_000 });
}
...
test.describe("Notifications (Seq 6 C4)", () => {
  test("P1 admin: /app/notifications loads", async ({ page }) => {
    await loginAs(page, "admin@coheron.com");
    await expectNoRuntimeCrash(page, "/app/notifications");
  });
});
