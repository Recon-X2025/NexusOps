...
- **LIVE on `connect.coheron.tech` = `origin/main` = `9cec822`** — migration head
  **`0074_ambiguous_rick_jones`** — verified through the terminal **`Deploy to Vultr`
  job of CI run `31253488299`** (all five jobs green: Lint · Unit & Integration · E2E ·
  Build Docker Images · Deploy to Vultr). `9cec822` is the PAN-audit workflow fix
  (workflow + docs only; identical app code).
- **⚠️ ORIGIN IS DELIBERATELY ONE COMMIT BEHIND LOCAL — this is NOT drift, do not
  "fix" it.** Local `main` carries one extra commit, **`5bd901d`**, a **docs-only
  exit-point refresh** kept local per standing rule 6 (no docs-only deploy). It rides
  origin with the next code change. So `git rev-list --left-right --count
  origin/main...HEAD` reads `0  1`, and origin records `9cec822` while local reads
  `5bd901d` — both describe the same live code. A previous session nearly looped
  "correcting" this; it is intentional.
...
**CI run `31253488299` — commit `9cec822` (PAN audit workflow reaches the api container
via `docker exec` instead of `docker compose`, fixing the `NEXUSOPS_WEB_IMAGE`
interpolation abort; workflow + docs only, IDENTICAL app code) — terminal `Deploy to
Vultr` job `success` — 2026-08-08 — migration head `0074_ambiguous_rick_jones` (no new
migration).** Verified via `gh run view 31253488299 --json jobs` (all five jobs green).
This is what is LIVE on `connect.coheron.tech`.

_The exit-point refresh is a docs-only commit (`5bd901d`) kept LOCAL and unpushed (rule 6);
it rides origin with the next code change, so local `main` reads one commit ahead of
origin — deliberate, not drift._

_Prior validated deploys (all superseded): `2bb3bac` (PAN edit-dialog fix) CI `31237849026`;
`5710dc2` (PAN at rest + null-structure) CI `31188691203`; `d979038` (C6) CI `31162786896`;
`e886a9c` (C2-STRUCT) CI `31147028089`; `209e537` (seed reconciler) CI `31141327969`;
`9960fc9` (C3 ESI) CI `31137358633`._
